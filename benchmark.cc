...
int32_t countPrimes2(uint32_t n) {
	...
	for (uint32_t i = 3; i <= n; i += 2) {
		for (uint32_t j = 3; j < (int) sqrt(i) ; j++)
			if (i % j == 0)
				goto notPrime;
		count++;
	notPrime: ; // do nothing if not prime
...
int main() {
	...
	benchmark1("b3", b3, n); // Conclude it is faster to count up than count down
	benchmark1("b4", b4, n); // compare b3 and b4. What can you conclude?
				 // Conclude similar to result above that it faster to count up than count down, but also that the adding factor causes a expontential growth in the time
	// what can you conclude from the difference between b1,b3 and b2,b4?
	// It is clear counting up through a loop is faster than counting down.  It is also clear that calling a void function takes much less time then computing the sum of two integers

  	const uint32_t n2 = 40000000; // 40 million
	benchmark2("b5", b5, n2);
	benchmark2("b6", b6, n2); // compare b5 and b6, what can you conclude?
				  // It is faster to calculate the sum of everything element in an array starting from the first rather than the last. 

	benchmark2("b7", b7, n2);
	benchmark2("b8", b8, n2); // compare b7 and b8, what can you conclude?
				  // When calculating the sum of array elements skipping by two, it is faster to start from the last element
	// what can you conclude from the difference between b5,b7 and b6,b8?
	// Making more For statments that have less repetitions takes less time

	// compare the times for going increasingly non-sequential. What happens?
	// Having For statements within another For statement takes more time than if there were just several different nonconnecting For statements.  In general the more non sequential a For statement is, the faster the action is performed, even if it takes more For loops
	benchmark2("b9", b9, n2);
